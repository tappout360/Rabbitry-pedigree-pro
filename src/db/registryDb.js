...
export async function performMigrationAndLoad() {
  ...
  migrationPromise = (async () => {
    ...
    const adminBreeders = await migrateOrLoadTable('rp_admin_breeders', db.adminBreeders, []);
    const rabbits = await migrateOrLoadTable('rp_rabbits', db.rabbits, []);
    const breedings = await migrateOrLoadTable('rp_breedings', db.breedings, []);
    const litters = await migrateOrLoadTable('rp_litters', db.litters, []);
    const ledger = await migrateOrLoadTable('rp_ledger', db.ledger, []);
    const shows = await migrateOrLoadTable('rp_shows', db.shows, []);
    const showEntries = await migrateOrLoadTable('rp_show_entries', db.showEntries, []);
    const chores = await migrateOrLoadTable('rp_chores', db.chores, []);
    const transfers = await migrateOrLoadTable('rp_transfers', db.transfers, []);
    
    const signatures = await migrateOrLoadTable('rp_signatures', db.signatures, []);
    const medical = await migrateOrLoadTable('rp_medical', db.medical, []);
    const weights = await migrateOrLoadTable('rp_weights', db.weights, []);
