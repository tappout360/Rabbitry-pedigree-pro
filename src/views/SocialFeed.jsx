...
import { Share2, Sparkles, MessageSquare, Heart, CheckCircle2, ShieldAlert, AlertCircle, Trash2, Eye } from 'lucide-react';
...
export default function SocialFeed({ currentUser, showToast }) {
  ...
  // Load posts from Dexie on mount
  useEffect(() => {
    loadPosts();
  }, []);
  ...
  const loadPosts = async () => {
    try {
      ...
      if (allPosts.length === 0) {
        // Seed some highly colorful mock posts
        const mockPosts = [
          {
            id: 'post-seed-1',
  ...
  const handleSubmitPost = async (e) => {
    ...
    try {
      // 1. Safe Automated scan for SSN/PHI and Profanity
      let clearedText = '';
      try {
      ...
      // 2. Parental Consent Checks
      const needsApproval = isChild;
      const newPost = {
      ...
      if (needsApproval) {
        showToast("Post submitted! Pending parental or coach safety approval before going public.", "info");
      } else {
  ...
  const handleApprovePost = async (postId) => {
    try {
      await db.socialPosts.update(postId, { parentApproved: true });
      setPosts(prev => prev.map(p => p.id === postId ? { ...p, parentApproved: true } : p));
  ...
  const filteredPosts = React.useMemo(() => {
    return posts.filter(p => {
      ...
      if (viewFilter === 'pending_approval') {
        return !p.parentApproved && (currentUser?.role === 'owner' || currentUser?.id === 'ab-admin');
      }
      // Public filter: shows approved posts, OR all posts belonging to the logged-in user
      return p.parentApproved || p.breederId === currentUser?.id;
    });
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          ...
          <div className="flex gap-2 border-b border-white/10 pb-1.5">
            ...
            {(currentUser?.role === 'owner' || currentUser?.id === 'ab-admin') && (
              ...
              >
                🛡️ Parent Approvals
                {posts.filter(p => !p.parentApproved).length > 0 && (
              ...
              <div key={p.id} className="glass-container p-4.5 flex flex-col gap-3 relative border border-white/5 hover:border-white/10 transition-all text-left">
                ...
                <div className="flex justify-between items-center border-t border-white/5 pt-2.5 mt-1 text-[10px] opacity-75 font-mono">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {!p.parentApproved && (currentUser?.role === 'owner' || currentUser?.id === 'ab-admin') && (
                      <button
                        onClick={() => handleApprovePost(p.id)}
                        className="py-1 px-2.5 bg-emerald-600 hover:bg-emerald-650 text-white font-bold border-none rounded-lg cursor-pointer flex items-center gap-1 shadow"
                      >
